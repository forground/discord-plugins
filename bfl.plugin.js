...
module.exports = (_ => {
    const config = {
        "info": {
            "name": "BFL",
            "author": "foreground",
            "description": "Adiciona funcionalidades extras na seção de Amigos da sua Interface do Client",
            "version": "1.5.3"
        }
    };
    ...
    return !window.BDFDB_Global || (!window.BDFDB_Global.loaded && !window.BDFDB_Global.started) ? class {
        ...
        getDescription () { return `O Plugin da Biblioteca necessário para ${config.info.name} está ausente. Abra as Configurações do Plugin e instale-o. \n\n${config.info.description}`; }

        downloadLibrary () {
            require("request").get("https://mwittrien.github.io/BetterDiscordAddons/Library/0BDFDB.plugin.js", (e, r, b) => {
                if (!e && b && r.statusCode == 200) require("fs").writeFile(require("path").join(BdApi.Plugins.folder, "0BDFDB.plugin.js"), b, _ => BdApi.showToast("Download da Biblioteca de BDFDB finalizada", { type: "success" }));
                else BdApi.alert("Erro", "Não foi possível baixar o Plugin de Biblioteca de BDFDB. Tente novamente mais tarde ou instale manualmente do GitHub.");
            });
        }
        ...
        load () {
            ...
            if (!window.BDFDB_Global.downloadModal) {
                ...
                BdApi.showConfirmationModal("Biblioteca Ausente", `O Plugin da Biblioteca necessário para ${config.info.name} está ausente. Abra as Configurações do Plugin e instale-o.`, {
        ...
        getSettingsPanel () {
            ...
            template.innerHTML = `<div style="color: var(--header-primary); font-size: 16px; font-weight: 300; white-space: pre; line-height: 22px;">O Plugin de Biblioteca necessário para ${config.info.name} está ausente.\nClique em <a style="font-weight: 500;">Baixar Agora</a> para instalá-lo.</div>`;
